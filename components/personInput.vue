<template>
	<view class="personinput comm">
		...
		<view class="person-input mt30 flex flex_al-cen font500 font30 color666">
			<input type="text" v-if="idNum!==2 && (idNum!==4) && (IdCard!==20)" :value="userinfoList.placeholder" @input="GetVal" :focus='true' @focus="GetVal"   />
			<input type="number" v-if="idNum==2 && (IdCard!==20) " :value="userinfoList.placeholder" @input="GetVal" :focus='true'  @focus="GetVal"/>
			<input type="number" v-if="idNum==4 " @input="GetVal" :value="userinfoList.placeholder" />
			<input type="number" v-if="IdCard==20 &&  (idNum!==4)" :value="userinfoList.placeholder" @input="GetVal" :focus='true'  @focus="GetVal"/>
	...
	export default {
		...
		methods: {
			...
			handKeep(item) {
				...
				if (this.idNum == 2 && (!/^1[34578]\d{9}$/.test(item.placeholder.trim()))) {
					uni.showToast({
						title: '手机格式不正确',
						icon: 'none',
						duration: 2000
